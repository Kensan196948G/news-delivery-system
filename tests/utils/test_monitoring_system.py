...
from src.utils.monitoring_system import (
    MonitoringSystem, ErrorCategory, APIUsageMetrics, AlertEvent, get_monitoring_system
)
...
@pytest.fixture
def monitoring_system(mock_config, mock_cache, mock_error_notifier, mock_performance_monitor):
    """MonitoringSystemインスタンス"""
    with patch('src.utils.monitoring_system.get_config', return_value=mock_config), \
         patch('src.utils.monitoring_system.get_cache_manager', return_value=mock_cache), \
         patch('src.utils.monitoring_system.get_error_notifier', return_value=mock_error_notifier), \
         patch('src.utils.monitoring_system.get_performance_monitor', return_value=mock_performance_monitor):
...
class TestMonitoringSystem:
    ...
    def test_classify_error(self, monitoring_system):
        ...
        assert monitoring_system._classify_error(rate_limit_error) == ErrorCategory.API_RATE_LIMIT
        
        # 認証エラー
        auth_error = Exception("401 Unauthorized")
        assert monitoring_system._classify_error(auth_error) == ErrorCategory.AUTHENTICATION_ERROR
        
        # ネットワークエラー
        network_error = Exception("Connection timeout")
        assert monitoring_system._classify_error(network_error) == ErrorCategory.NETWORK_ERROR
        
        # データ形式エラー
        json_error = Exception("JSON decode error")
        assert monitoring_system._classify_error(json_error) == ErrorCategory.DATA_FORMAT_ERROR
        
        # ディスク容量エラー
        disk_error = Exception("No space left on device")
        assert monitoring_system._classify_error(disk_error) == ErrorCategory.DISK_SPACE_ERROR
        
        # その他エラー
        other_error = Exception("Unknown error")
        assert monitoring_system._classify_error(other_error) == ErrorCategory.SYSTEM_ERROR
    ...
    async def test_handle_error_with_classification(self, monitoring_system):
        ...
        with patch.object(monitoring_system, '_update_error_statistics') as mock_update:
            category = await monitoring_system.handle_error_with_classification(test_error, "test_context")
            
            assert category == ErrorCategory.API_RATE_LIMIT
            mock_update.assert_called_once_with(ErrorCategory.API_RATE_LIMIT, "test_context")
    ...
    def test_update_error_statistics(self, monitoring_system, mock_cache):
        ...
        monitoring_system._update_error_statistics(ErrorCategory.NETWORK_ERROR, "test_context")
    ...
    async def test_create_duplicate_alert(self, monitoring_system):
        """重複アラート防止テスト"""
        # 最初のアラート
        await monitoring_system._create_alert("dup_alert", "category", ErrorSeverity.MEDIUM, "Message 1", {})
        
        # 重複アラート（作成されないはず）
        await monitoring_system._create_alert("dup_alert", "category", ErrorSeverity.MEDIUM, "Message 2", {})
    ...
    async def test_resolve_alert(self, monitoring_system):
        """アラート解決テスト"""
        # アラート作成
        await monitoring_system._create_alert("resolve_test", "category", ErrorSeverity.MEDIUM, "Test", {})
...
class TestIntegrationScenarios:
    ...
    async def test_full_error_handling_workflow(self, monitoring_system):
        ...
        with patch.object(monitoring_system, '_create_alert') as mock_alert:
            ...
            for error, context in errors:
                category = await monitoring_system.handle_error_with_classification(error, context)
    ...
    async def test_monitoring_loop_simulation(self, monitoring_system):
        """監視ループシミュレーション"""
        # ヘルスチェック実行
        with patch.object(monitoring_system, '_perform_health_check') as mock_health_check:
