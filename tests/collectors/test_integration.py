...
class TestCollectorIntegration:
    ...
    async def test_concurrent_collection(self, mock_config, mock_logger):
        ...
        with patch.object(
            ...
            return_value=newsapi_response,
        ), patch.object(
            gnews,
            "fetch_with_cache",
            new_callable=AsyncMock,
            return_value=gnews_response,
        ), patch.object(
            nvd, "fetch_with_cache", new_callable=AsyncMock, return_value=nvd_response
        ):
            # 並行実行
            tasks = [
    ...
    async def test_error_handling_resilience(self, mock_config, mock_logger):
        ...
        with patch.object(
            ...
            gnews, "fetch_with_cache", new_callable=AsyncMock, return_value=None
        ), patch.object(
            nvd,
            "fetch_with_cache",
            new_callable=AsyncMock,
