...
class NVDCollector(BaseCollector):
    ...
    def __init__(self, config, logger):
        ...
        # NVD固有設定
        self.cvss_threshold = config.get('news_sources', 'nvd', 'cvss_threshold', default=7.0)
        self.daily_limit = config.get('news_sources', 'nvd', 'rate_limit_per_day', default=50)
    ...
    async def collect(self, days_back: int = 7, 
                     cvss_severity: str = 'HIGH,CRITICAL') -> List[Article]:
        """NVDから脆弱性情報を収集"""
        ...
        try:
            self.logger.info(f"Fetching NVD vulnerabilities: {days_back} days back, severity={cvss_severity}")
            
            # APIリクエスト実行（タイムアウト延長）
            data = await self.fetch_with_cache(self.base_url, params, cache_ttl=7200)  # 2時間キャッシュ
            ...
            self.logger.info(f"NVD returned {len(vulnerabilities)} vulnerabilities (total: {total_results})")
            
            # 脆弱性変換
            articles = self._process_vulnerabilities(vulnerabilities)
            
            self.request_count += 1
            self.logger.info(f"NVD: Successfully collected {len(articles)} vulnerability articles")
    ...
    def _process_vulnerabilities(self, vulnerabilities: List[Dict[str, Any]]) -> List[Article]:
        """脆弱性データの処理・変換"""
        processed_articles = []
    ...
    def _create_vulnerability_article(self, cve: Dict[str, Any], cve_id: str) -> Optional[Article]:
        """脆弱性記事オブジェクトの作成"""
        try:
            # CVSSスコア取得
            ...
            # 重要度スコア計算
            importance_score = self._calculate_importance_score(cvss_score, severity, cwe_info)
            
            # 記事内容構築
            content = self._build_vulnerability_content(
                cve_id, description, cvss_score, cvss_vector, 
                cwe_info, references, severity
            )
            ...
            self.logger.error(f"Failed to create vulnerability article for {cve_id}: {e}")
            return None
    ...
    def _calculate_importance_score(self, cvss_score: float, 
                                   severity: str, cwe_info: List[str]) -> int:
        """重要度スコア計算"""
        # CVSSスコアベース
        if cvss_score >= 9.0:
            score = 10
    ...
    def _build_vulnerability_content(self, cve_id: str, description: str, 
                                   cvss_score: float, cvss_vector: str,
                                   cwe_info: List[str], references: List[str],
                                   severity: str) -> str:
        """脆弱性記事の内容を構築"""
        content_parts = [
            f"脆弱性ID: {cve_id}",
        ...
        if cwe_info:
            content_parts.extend([
                "脆弱性タイプ:",
                ", ".join([f"{cwe} ({self.cwe_categories.get(cwe, 'その他')})" for cwe in cwe_info]),
                ""
            ])
        
        if cvss_vector:
            content_parts.extend([
                f"CVSSベクトル: {cvss_vector}",
                ""
            ])
