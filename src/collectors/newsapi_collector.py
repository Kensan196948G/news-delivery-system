...
class NewsAPICollector(BaseCollector):
    ...
    def __init__(self, config, logger):
        ...
        self.daily_limit = config.get(
            "news_sources", "newsapi", "rate_limit_per_day", default=1000
        )
    ...
    async def collect(
        ...
        try:
            self.logger.info(
                f"Fetching NewsAPI articles: category={category}, country={country}"
            )
            ...
            self.logger.info(
                f"NewsAPI returned {len(articles_data)} articles (total: {total_results})"
            )
            ...
            self.logger.info(
                f"NewsAPI: Successfully collected {len(articles)} articles"
            )
    ...
    async def collect_everything(
        ...
        try:
            ...
            self.request_count += 1
            self.logger.info(
                f"NewsAPI Everything: Successfully collected {len(articles)} articles"
            )
    ...
    def _process_articles(
        self, articles_data: List[Dict[str, Any]], category: str, country: str
    ) -> List[Article]:
        """記事データの処理・変換"""
        processed_articles = []
    ...
    def _create_article(
        self, article_data: Dict[str, Any], category: str, country: str
    ) -> Optional[Article]:
        """記事オブジェクトの作成"""
        try:
            # ソース情報
    ...
    def _infer_category_from_query(self, query: str) -> str:
        ...
        if any(
            ...
            keyword in query_lower
            for keyword in ["business", "economy", "market", "finance"]
        ):
            return "business"
        elif any(
            keyword in query_lower for keyword in ["health", "medical", "healthcare"]
        ):
            return "health"
        elif any(
            keyword in query_lower for keyword in ["security", "cyber", "vulnerability"]
        ):
