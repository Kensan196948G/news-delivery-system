...
from .base_collector import APIError, BaseCollector, CollectionError
...
class GNewsCollector(BaseCollector):
    ...
    def __init__(self, config, logger):
        ...
        self.daily_limit = config.get(
            "news_sources", "gnews", "rate_limit_per_day", default=100
        )
    ...
    async def collect(
        ...
        try:
            if query:
                ...
                self.logger.info(
                    f"Fetching GNews headlines: category={category}, country={country}"
                )
            ...
            self.logger.info(
                f"GNews returned {len(articles_data)} articles (total: {total_articles})"
            )
    ...
    async def collect_human_rights_news(self, max_results: int = 30) -> List[Article]:
        ...
        for keyword in self.human_rights_keywords[:5]:  # 上位5キーワードで検索
            try:
                ...
                self.logger.warning(
                    f"Failed to collect human rights news for '{keyword}': {e}"
                )
                continue
        ...
        self.logger.info(
            f"Collected {len(unique_articles)} unique human rights articles"
        )
    ...
    def _create_article(
        self, article_data: Dict[str, Any], category: str, query: Optional[str] = None
    ) -> Optional[Article]:
        """記事オブジェクトの作成"""
        try:
            # ソース情報
            ...
            source_name = (
                source.get("name", "GNews") if isinstance(source, dict) else str(source)
            )

            # カテゴリ決定
            if query and any(
                keyword in query.lower() for keyword in self.human_rights_keywords
            ):
                ...
                article_category = self.category_mapping.get(
    ...
    async def collect_by_query(
        self, query: str, count: int, language: str = "en", country: str = "us"
    ) -> List[Article]:
        """クエリによる記事収集（main.py統合用）"""
        return await self.collect(
            query=query, language=language, country=country, max_results=count
        )

    async def collect_by_category(
        self, category: str, count: int, language: str = "en", country: str = "us"
    ) -> List[Article]:
        """カテゴリによる記事収集（main.py統合用）"""
        return await self.collect(
            category=category, language=language, country=country, max_results=count
        )
