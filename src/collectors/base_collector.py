...
class BaseCollector(ABC):
    ...
    def __init__(self, config, logger, service_name: str):
        ...
        # API設定
        self.api_key = (
            config.get_api_key(service_name) if hasattr(config, "get_api_key") else None
        )
    ...
    async def fetch_with_cache(
        self, url: str, params: Dict[str, Any], cache_ttl: int = 3600
    ...
    def enhanced_validate_article_data(
        ...
        try:
            ...
            if len(title) > self.validation_rules["max_title_length"]:
                validation_result["warnings"].append(
                    f"Title very long: {len(title)} characters"
                )
                # 長すぎる場合は切り詰めるが有効とする

            # コンテンツ検証
            content = article_data.get("content", "") or article_data.get(
                "description", ""
            )
            if content and len(content) < self.validation_rules["min_content_length"]:
                validation_result["warnings"].append(
                    f"Content short: {len(content)} characters"
                )
            ...
            if hasattr(self, "_collected_hashes"):
                ...
                if content_hash in self._collected_hashes:
                    validation_result.update(
                        {"is_valid": False, "reason": "Duplicate content detected"}
                    )
                    return validation_result
                else:
    ...
    def get_collection_statistics(self) -> Dict[str, Any]:
        ...
        if stats["articles_collected"] > 0:
            stats["articles_per_request"] = stats["articles_collected"] / max(
                stats["successful_requests"], 1
            )
            stats["filter_rate"] = (
                stats["articles_filtered"] / stats["articles_collected"]
            ) * 100
        else:
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            health_status["checks"]["rate_limit"] = {
                "status": "ok"
                if not rate_status.get("is_limited", False)
                else "warning",
