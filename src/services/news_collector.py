...
from utils.monitoring_integration import (
    MonitoredCollectorMixin, get_health_checker, monitor_collection
)
from models.database import Database
from models.article import Article
from ..collectors import NewsAPICollector, GNewsCollector, NVDCollector
...
class NewsCollector:
    ...
    def __init__(self, config=None):
        ...
        self.newsapi = NewsAPICollector(self.config, self.logger) if self.config.is_service_enabled('newsapi') else None
        
        # GNews is optional - disable if API key issues
        try:
            if self.config.is_service_enabled('gnews') and self.config.get_api_key('gnews'):
                self.gnews = GNewsCollector(self.config, self.logger)
                self.logger.info("GNews collector initialized successfully")
            else:
                self.gnews = None
                self.logger.info("GNews collector disabled - API key not available or service disabled")
        except Exception as e:
            self.logger.warning(f"GNews collector initialization failed: {e}")
            self.gnews = None
        
        self.nvd = NVDCollector(self.config, self.logger) if self.config.is_service_enabled('nvd') else None
        
        # コレクターのヘルスチェック登録
        asyncio.create_task(self._register_collectors_for_monitoring())
    ...
    async def collect_all_news(self) -> List[Article]:
        ...
        try:
            ...
            if self.newsapi:
                tasks.append(self.newsapi.collect(category='general', country='jp', language='ja', page_size=30))
                tasks.append(self.newsapi.collect(category='technology', country='us', language='en', page_size=20))
                tasks.append(self.newsapi.collect(category='business', country='us', language='en', page_size=15))
            
            # GNews - Additional international news with focus on human rights
            if self.gnews:
                tasks.append(self.gnews.collect(category='world', language='en', max_results=20))
    ...
    async def collect_emergency_news(self) -> List[Article]:
        ...
        try:
            ...
            if self.newsapi:
                breaking_news = await self.newsapi.collect(category='general', country='jp', language='ja', page_size=10)
    ...
    async def collect_by_category(self, category: str, max_articles: int = 50) -> List[Article]:
        """カテゴリ別ニュース収集"""
        try:
            articles = []
            
            if category == 'domestic_social' and self.newsapi:
                articles.extend(await self.newsapi.collect(
                    category='general', country='jp', language='ja', page_size=max_articles
                ))
            
            elif category == 'international_social' and self.gnews:
                articles.extend(await self.gnews.collect_human_rights_news(max_results=max_articles))
            
            elif category == 'tech' and self.gnews:
                articles.extend(await self.gnews.collect_tech_news(max_results=max_articles))
                ...
                tasks = []
                if self.newsapi:
                    tasks.append(self.newsapi.collect(category='business', country='jp', language='ja', page_size=max_articles//2))
                    tasks.append(self.newsapi.collect(category='business', country='us', language='en', page_size=max_articles//2))
    ...
    def _filter_and_rank_articles(self, articles: List[Article]) -> List[Article]:
        ...
        for article in articles:
            ...
            if any(keyword in title.lower() for keyword in ['ad', 'sponsored', 'advertisement']):
                continue
            ...
            min_importance = self.config.get('article_processing', 'importance_threshold', default=3)
            if importance_score >= min_importance:
                filtered_articles.append(article)
        
        # Sort by importance score (descending)
        filtered_articles.sort(key=lambda x: getattr(x, 'importance_score', 0), reverse=True)
    ...
    def _calculate_importance_score(self, article) -> int:
        ...
        if hasattr(article, 'title'):  # Article dataclass
            ...
            category = str(article.category) if hasattr(article, 'category') and article.category else ''
        ...
        tech_keywords = ['ai', 'artificial intelligence', '人工知能', 'セキュリティ', 'security', 
                        'vulnerability', '脆弱性', 'breach', 'hack', 'cyber']
    ...
    async def get_monitoring_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            collector_stats = {}
            for name, collector in [('newsapi', self.newsapi), ('gnews', self.gnews), ('nvd', self.nvd)]:
