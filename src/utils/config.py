...
class ConfigManager:
    ...
    def __init__(
        self, config_path: Optional[str] = None, env_path: Optional[str] = None
    ):
        ...
        self.config_data = self.config  # backward compatibility
    ...
    def get_data_path(self, *paths: str) -> Path:
        """Generate data path within project data directory"""
        base = Path(self.get("paths", "data", default=self.project_root / "data"))
        full_path = base.joinpath(*paths)
    ...
    def get_api_key(self, service: str) -> str:
        ...
        if service == "nvd":
            return api_key or ""  # NVD does not require API key
        if not api_key:
    ...
    def get_storage_path(self, path_type: str) -> Path:
        ...
        base_path = Path(
            self.get(
                "storage", "external_hdd_path", default=str(self.project_root / "data")
            )
        )
