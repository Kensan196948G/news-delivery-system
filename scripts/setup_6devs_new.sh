...
if [ "$PANE_COUNT" -eq 8 ]; then
    ...
    if [ -f "$INSTRUCTIONS_DIR/manager.md" ]; then
        # 各ペインで環境変数を設定してからClaude起動（認証高速化付き）
        echo "🚀 Claudeエージェント高速起動中..."
        ...
        tmux send-keys -t $SESSION:0.4 "export CLAUDE_CODE_CONFIG_PATH='$HOME/.local/share/claude' && export CLAUDE_CODE_CACHE_PATH='$HOME/.cache/claude' && claude --continue --dangerously-skip-permissions --model sonnet '$INSTRUCTIONS_DIR/developer.md'" C-m &
        tmux send-keys -t $SESSION:0.5 "export CLAUDE_CODE_CONFIG_PATH='$HOME/.local/share/claude' && export CLAUDE_CODE_CACHE_PATH='$HOME/.cache/claude' && claude --continue --dangerously-skip-permissions --model sonnet '$INSTRUCTIONS_DIR/developer.md'" C-m &
        tmux send-keys -t $SESSION:0.6 "export CLAUDE_CODE_CONFIG_PATH='$HOME/.local/share/claude' && export CLAUDE_CODE_CACHE_PATH='$HOME/.cache/claude' && claude --continue --dangerously-skip-permissions --model sonnet '$INSTRUCTIONS_DIR/developer.md'" C-m &
        tmux send-keys -t $SESSION:0.7 "export CLAUDE_CODE_CONFIG_PATH='$HOME/.local/share/claude' && export CLAUDE_CODE_CACHE_PATH='$HOME/.cache/claude' && claude --continue --dangerously-skip-permissions --model sonnet '$INSTRUCTIONS_DIR/developer.md'" C-m &
        
        # 並列処理完了を待機
        wait
        
        echo "✅ Claude高速起動完了（認証時間短縮）"
