...
# Delete All Workflow Runs Script (Fixed)
# 全てのワークフロー実行履歴を削除します

echo "🗑️ GitHub Workflow実行履歴削除スクリプト (修正版)"
echo "=================================================="
...
show_progress() {
    ...
    printf "] %3d%% (%d/%d) ✅:%d ❌:%d" $percent $current $total $DELETED $FAILED
}
...
for run_id in $ALL_RUNS; do
    ...
    # 削除実行
    if gh run delete $run_id --repo "$OWNER/$REPO" >/dev/null 2>&1; then
        ((DELETED++))
    else
        ((FAILED++))
    fi
...
echo "  ✅ 削除成功: $DELETED"
echo "  ❌ 削除失敗: $FAILED"
echo "  成功率: $((DELETED * 100 / TOTAL))%"
echo ""

# 残っている実行履歴を確認
echo "📊 現在の実行履歴を確認中..."
REMAINING=$(gh run list --repo "$OWNER/$REPO" --limit 100 --json databaseId --jq '. | length' 2>/dev/null)

if [ -n "$REMAINING" ]; then
    echo "  残り: $REMAINING 件"
    if [ "$REMAINING" -gt 0 ]; then
        echo "  💡 残った履歴があります。再実行するか手動で削除してください"
    fi
else
    echo "  確認できませんでした"
fi
...
echo "2. 残りがある場合は再実行: ./scripts/delete-all-runs-fixed.sh"
echo "3. 新しいワークフローを実行できます"